		...
		<div class="row justify-content-center">
			<div class="col-md-6 col-lg-5">
				...
				>
					...
					<template slot="body">
						<tr v-for="(lista,index) in ListaCaja" :key="index">
							<td>{{ lista.Nombre }}</td>
							<td>
								<Cbtnaccion
									:isModal="EsModal"
						...
						<CSinRegistros :pContIF="ListaCaja.length" :pColspan="2" ></CSinRegistros>
					</template>
				</Clist>
			</div>
		</div>
...
import Form from "@/views/catalogos/caja/Form.vue";
	...
	methods: {
		...
		Eliminar(Id) {
			this.$swal({
				...
				if (result.value) {
					this.$http.delete("caja/" + Id).then(res => {
