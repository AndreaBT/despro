<template>
    <div>
		<Clist :regresar="true" @FiltrarC="Lista" :Filtro="Filtro"  :Nombre="NameList" :Pag="Pag"
			...
			<template slot="header">
             ...
             <template slot="body">
				 <tr v-for="(lista,index) in ListaTipoServicio" :key="index" >
					 <td>{{lista.Concepto }}</td>
					 <td>{{lista.GrossM}}</td>
					 <td>{{lista.TipoServicio}}</td>
					 <td><input type="color" disabled="true" :value="lista.Color"></td>
					 <td>
						 <Cbtnaccion :isModal="EsModal" :Id="lista.IdTipoSer" :IrA="FormName" >
							 <template slot="btnaccion">
							 </template>
						 </Cbtnaccion>
					 </td>
				 </tr>
				 <CSinRegistros :pContIF="ListaTipoServicio.length" :pColspan="5" ></CSinRegistros>
            </template>
...
import Form from '@/views/catalogos/tiposervicio/Form.vue'
import CSinRegistros from "../../../components/CSinRegistros";

export default {
    name :'listConfTipoServicio',
    components :{
        Modal,
        Clist,
        Cbtnaccion,
