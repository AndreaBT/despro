	...
	<div>
		<CHead :oHead="Head">
			<template slot="component">
				...
				<button v-if="!isVisible" @click="mostrarFiltros('open')" type="button" class="btn btn-01 mb-2 mr-1 filtro">
					Filtros
				</button>

				<button v-else @click="mostrarFiltros('close')" type="button" class="btn btn-01 mb-2 mr-1 salir">
					Filtros
				</button>
			</template>
		</CHead>
		...
		<Clist :regresar="true" @FiltrarC="Lista" :pShowBtnAdd="true" :ShowHead="false"
			...
			<template slot="body">
				<tr v-for="(lista, key, index) in ListaCtaPorPagar" :key="index">
					<td>{{ lista.Departamento }}</td>
					<td>{{ lista.Descripcion }}</td>
					<td>{{ lista.Cuenta }}</td>
					<td>{{ lista.FechaFactura }}</td>
					<td>{{ lista.FechaPago }}</td>
					<td v-show="TipoFiltro != 'NO'">{{ lista.FechaRealPago }}</td>
					<td>$ {{ Number(lista.Monto).toLocaleString() }}</td>
					<td>{{ lista.NumFactura }}</td>
					<td>{{ lista.Credito }}</td>
					<td><b>{{ lista.Estatus }}</b></td>
					<td class="badge badge-pill badge-primary mt-2" v-if="lista.Vigencia == 'No vencido'">
						{{ lista.Vigencia }}
					</td>
					<td class="badge badge-pill badge-Vigencia mt-2" v-if="lista.Vigencia == 'Vencido'">
						{{ lista.Vigencia }}
					</td>
					<td>
						
						<Cbtnaccion   v-if="lista.Estatus == 'NO'" :isModal="EsModal" :Id="lista.IdCtaPagar" :IrA="FormName">
							<template slot="btnaccion">
								<button v-if="lista.Estatus == 'NO'" v-b-tooltip.hover.leftbottom title="Pagado" @click="Observation(lista.IdCtaPagar)" data-toggle="modal" data-target="#Observacion" data-backdrop="static" data-keyboard="false" type="button" class="btn-icon mr-2">
									<i class="fas fa-dollar-sign"></i>
								</button>
								<button v-if="lista.Factura != ''" @click="openInvoice(lista)" v-b-tooltip.hover.top title="Factura" type="button" class="btn-icon mr-2">
									<i class="fas fa-file-pdf"></i>
								</button>
								<button v-if="lista.ArchivoUno != ''" @click="openEvidenceOne(lista)" v-b-tooltip.hover.top title="Evidencia" type="button" class="btn-icon mr-2">
									<i class="fas fa-file-pdf"></i>
								</button>
								<button v-if="lista.ArchivoDos != ''" @click="openEvidenceTwo(lista)" v-b-tooltip.hover.top title="Evidencia" type="button" class="btn-icon mr-2">
									<i class="fas fa-file-pdf"></i>
								</button>
							</template>
						</Cbtnaccion>
						...
						<Cbtnaccion  v-if="lista.Estatus != 'NO'" :isModal="EsModal" :Id="lista.IdCtaPagar" :IrA="FormName">
							...
							<template  slot="btnaccion">
								<button v-if="lista.Estatus != 'NO'" v-b-tooltip.hover.leftbottom title="Observación" @click="Observation(lista.IdCtaPagar)" data-toggle="modal" data-target="#Observacion" data-backdrop="static" data-keyboard="false" type="button" class="btn-icon mr-2">
									<i class="fas fa-info-circle"></i>
								</button>
								<button v-if="lista.Factura != '' && lista.Estatus != 'NO'" @click="openInvoice(lista)" v-b-tooltip.hover.top title="Factura" type="button" class="btn-icon mr-2">
									<i class="fas fa-file-pdf"></i>
								</button>
								<button v-if="lista.ArchivoUno != ''  && lista.Estatus != 'NO'" @click="openEvidenceOne(lista)" v-b-tooltip.hover.top title="Evidencia" type="button" class="btn-icon mr-2">
									<i class="fas fa-file-pdf"></i>
								</button>
								<button v-if="lista.ArchivoDos != ''  && lista.Estatus != 'NO'" @click="openEvidenceTwo(lista)" v-b-tooltip.hover.top title="Evidencia" type="button" class="btn-icon mr-2">
									<i class="fas fa-file-pdf"></i>
								</button>
							</template>
						</Cbtnaccion>
				...
				<CSinRegistros :pContIF="ListaCtaPorPagar.length" :pColspan="[TipoFiltro !== 'NO' ? 12 : 11]" />
			</template>
		...
		<Modal :NameModal="'Observacion'" :poBtnSave="oBtnSave2" :size="size" :Nombre="'Observación'">
			<template slot="Form">
...
import Observacion from "@/views/modulos/ctaporcobrarpagar/ctaporpagaradmin/Observacion.vue";
import Form from "@/views/modulos/ctaporcobrarpagar/ctaporpagaradmin/Form.vue";
...
export default {
	...
	data() {
		return {
			...
			size: "modal-lg",
			NameList: "Administración Cuentas Por Pagar",
			ListaCtaPorPagar: [],
			ListaHeader: [],
			SuppliersList: [],
			rangeDate: {},
			...
			Head: {
				ShowHead: true,
				Title: "Administración Cuentas Por Pagar",
				BtnNewShow: true,
				BtnNewName: "Nuevo",
				isreturn: true,
				isModal: true,
	...
	methods: {
		...
		getTooTip(lista) {
			...
			if (lista.Sucursal != "") {
				html =
					"Servicio : " + lista.FechaI2 + " Prefactura : " + lista.FechaReg;
			}
			return html;
		},
		get_Suppliers() {
