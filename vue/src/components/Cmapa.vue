<template>
    <div>
        ...
        -->
        <GmapMap :center="center" :zoom="zoom" ref="gmap" id="gmap" map-type-id="hybrid" style="width: 780px; height: 500px" :mapTypeControl="false" :scaleControl="false" :streetViewControl="false" :rotateControl="false" :fullscreenControl="true" :disableDefaultUi="false">
            ...
            <GmapMarker ref="marker" v-for="(item, index) in arreglodinamico" :key="index" :position="item.position" :clickable="true" :draggable="false" :icon="validateicon(item)" @click="toggleInfoWindow(item, index);center=item.position" @mouseover="toggleInfoWindowMouse(item, index);position=item.position"/>

            <!-- *Eventos de Gmap @mouseout="toggleInfoWindow(item, index);center=item.position" @mouseover="toggleInfoWindow(item, index);center=item.position" @click="toggleInfoWindow(item, index);center=item.position" :title="getTooTip(item)" :icon=" item.datos.Estatus =='Disponible' ? { url:  require('@/style/images/Cliente.png')} : { url:  require('@/style/images/Ocupado.png')}" -->
...
export default {
    ...
    data() {
        return {
            ...
            markers: [],
            center: {
                lat:19.43441642213732, 
                lng:-99.13307696649245
            },
            datos:{
                Tecnico:'',
                Cliente:'',
            ...
            infoOptions: { //optional: offset infowindow so it visually sits nicely on top of our marker
                pixelOffset: {
                    width: 0,
                    height: -35
                }
