...
class ReporteServicios extends Cbasereport
{
    function Header()
    {
    ...
    function contenido(){
		...
		#FIRMA
        //if($this->GetY()>265){//si y es mayo a 270 sasltar firma
            //$this->AddPage();
        //}
		...
		if($this->getDatos()['statusimg']){
			...
			if($Firma->Firma2 != ''){
				$this->Image('assets/files/firmas/'.$ID_EMPRESA.'/'.$ID_SUCURSAL.'/'.$Firma->Firma2,$x+75, 219,40,20);
				...
				//$this->Image($img,$x+80, 243,40,20);

				// LINEA DE FIRMA
				/*$y = $this->getY()+18;
				$style = array('width' => 0.5, 'cap' => 'butt', 'join' => 'miter', 'dash' => 0, 'color' => array(1,113,122));
				$this->Line($x+65, $y+3, 130, $y+3, $style);*/
			}
		}
